...
const App = () => {
  ...
  const handleSort = (parameter) => {
    const sortedTasks = tasks.sort((a, b) => {
      // First compare by the main parameter, typically 'dueDate'
      if (a[parameter] < b[parameter]) {
        return -1;
      }
      if (a[parameter] > b[parameter]) {
      ...
      // If the main parameter is 'dueDate' and the dates are the same, then sort by 'dueTime'
      if (parameter === 'dueDate' && a[parameter] === b[parameter]) {
      ...
      // Return 0 if both are identical
      return 0;
    });
  
    // Use spread to trigger a state update in case the original order didn't change
    setTasks([...sortedTasks]);
  };
  ...
  const handleEditDueTime = (id, newDueTime) => {
    const updatedTasks = tasks.map(task =>
      task.id === id ? { ...task, dueTime: newDueTime } : task
    );
    setTasks(updatedTasks);
  };
      ...
      <div className="mx-auto w-full mb-10">
        <div className='sticky top-0 py-4 mx-auto bg-white w-full shadow-lg border-b-2 border-dashed border-blue-300'>
          ...
          <form onSubmit={handleSubmit} className="mb-4 flex flex-wrap justify-center items-center flex-row gap-4">
            ...
            <input type="time" name="dueTime" value={newTask.dueTime} onChange={handleChange} className="text-lg font-semibold mr-2 border border-gray-300 rounded-full px-4 py-2" />
          ...
          {tasks.map(task => (
            ...
            >
              <div className='flex flex-row gap-4 items-center flex-wrap'>
                  ...
                  <input type="time" value={task.dueTime} onChange={(e) => handleEditDueTime(task.id, e.target.value)} className="text-sm text-gray-500 mr-2 border-b border-gray-300 rounded-full px-4 py-3" />
